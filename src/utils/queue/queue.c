...
queue_h Queue_Create(size_t const len, size_t const item_size) {
   assert(IS_POWER_OF_2(len));

   queue_t *inst = calloc(1, sizeof(queue_h));
   assert(inst);

   uint8_t *buf = calloc(len + 1, item_size);
   assert(buf);
...
error_t Queue_Init(queue_t *const queue, queue_init_t const *const config) {
   ASSERT(queue);
   ASSERT(config);
...
bool Queue_Is_Full(queue_t const *const queue) {
   ASSERT(queue);
...
bool Queue_Is_Empty(queue_t const *const queue) {
   ASSERT(queue);

   return (queue->index == queue->outdex);
}

error_t Queue_Push(queue_h *const queue, void const *const src_item) {
   ASSERT(queue);
   ASSERT(src_item);
...
error_t Queue_Pop(queue_t *const queue, void *const dest_item) {
   ASSERT(queue);
   ASSERT(dest_item);
...
void Queue_Flush(queue_t *const queue) {
   queue->index = 0;
   queue->outdex = 0;
}

static size_t Next_Index(queue_t const *const queue, size_t dex) {
   dex++;
