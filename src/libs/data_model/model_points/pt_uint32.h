...
error_t pt_uint32_observer_attach (
    pt_uint32_t * const mp, mp_subsciber_uint32_t const * const observer, uint16_t initial_seq_num
);
...
// returns success if read value is valid, returns the current sequence
// number through parameter if not null
