...
#define CRITICAL_SECTION(IRQ_ID)                                                     \
   DEFER(bool UNIQUE_VAR_NAME(_int_state_) = Core_Interrupts_SaveAndDisable(IRQ_ID), \
...
typedef enum { IRQ_ALL = SIZE_MAX } core_irq_t;

typedef enum { CORE_CB_SYSTICK, CORE_CB_MAX } core_cb_t;
...
error_t Core_Init(void);
void Core_DeInit(void);

error_t Core_Clock_Init(core_clock_config_t const* const config);

error_t Core_Irq_Callback_Attach(core_irq_t const irq_id, void* cb);

void Core_Irq_Disable_All(void);
